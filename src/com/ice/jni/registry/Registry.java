...
** 
 * Modified by Daniel Dreibrodt to compile safely with JDK 1.5+
 *
*/
...
Registry
	...
	static
		...
		Registry.topLevelKeys = new Hashtable<String,RegistryKey>( 16 );
	...
	listKeysCommand( RegistryKey topKey, String keyName )
		...
		try {
			Enumeration enum_var = subKey.keyElements();
			for ( int kIdx = 0 ; enum_var.hasMoreElements() ; ++kIdx )
				{
				String keyStr = (String) enum_var.nextElement();
				System.err.println
					( "Subkey[" + kIdx + "] = '" + keyStr + "'" );
				}
			}
			...
			{
			System.err.println
				( "ERROR getting key enum_varerator, "
					+ ex.getMessage() );
			return;
			}
		}
	...
	listValuesCommand( RegistryKey topKey, String keyName )
		...
		try {
			Enumeration enum_var = subKey.valueElements();
			for ( int kIdx = 0 ; enum_var.hasMoreElements() ; ++kIdx )
				{
			...
			{
			System.err.println
				( "ERROR getting value enum_varerator, "
					+ ex.getMessage() );
			return;
			}
		}
